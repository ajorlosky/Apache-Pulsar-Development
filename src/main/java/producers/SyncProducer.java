...
import interceptors.CustomProducerInterceptor;
...
public class SyncProducer {
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		Producer<StockTicker> producer = pulsarClient.newProducer(JSONSchema.of(StockTicker.class))
				.topic(AppConfig.singleTopic).producerName("sync-producer").intercept(interceptor).enableBatching(true)
				...
				.blockIfQueueFull(true).maxPendingMessages(15000).create();
		...
		/*
		 * Option 1: The producer is sending messages in a round-robin fashion in the
		 * ticker-list order for a preset amount of time.
		 */

		if (continuous.equals("no"))
			sendNoncontinuousMessages(producer, symbols, stockTickers, update);
		/*
		 ...
		 */
			...
			sendContinuousMessages(producer, symbols, stockTickers, update);
		...
		closeResources(producer, pulsarClient);
	}

	// produces message using ticker index, producer, and arraylist of stock tickers
	// (updates and returns tickerIndex after)
	public static int produceMessage(Producer<StockTicker> producer, ArrayList<String> symbols,
		...
		try {
			...
			if (value != null) {
				producer.newMessage(JSONSchema.of(StockTicker.class)).key(key).value(value).send();
	...
	// a specific rate)
	public static void sendNoncontinuousMessages(Producer<StockTicker> producer, ArrayList<String> symbols,
	...
	// broker
	public static void sendContinuousMessages(Producer<StockTicker> producer, ArrayList<String> symbols,
	...
	public static void closeResources(Producer<StockTicker> producer, PulsarClient pulsarClient)
